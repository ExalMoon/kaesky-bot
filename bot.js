...
client.once('ready', async () => {
    ...
    try {
        await rest.put(Routes.applicationGuildCommands(CLIENT_ID, SUNUCU_ID), { 
            body: [
...
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.isCommand()) {
        ...
        if (interaction.commandName === 'şikayet') {
            ...
            modal.addComponents(
                new ActionRowBuilder().addComponents(
                    new TextInputBuilder().setCustomId('sikayet_nick').setLabel('Oyun İçi Kullanıcı Adınız').setStyle(TextInputStyle.Short).setRequired(true)
                ),
                new ActionRowBuilder().addComponents(
                    new TextInputBuilder().setCustomId('sikayet_edilen').setLabel('Şikayet Edilen Kullanıcı').setStyle(TextInputStyle.Short).setRequired(true)
                ),
                new ActionRowBuilder().addComponents(
                    new TextInputBuilder().setCustomId('sikayet_sebep').setLabel('Şikayet Sebebi').setStyle(TextInputStyle.Paragraph).setRequired(true)
                )
            );
        ...
        if (interaction.commandName === 'öneri') {
            ...
            modal.addComponents(
                new ActionRowBuilder().addComponents(
                    new TextInputBuilder().setCustomId('oneri_nick').setLabel('Oyun İçi Kullanıcı Adınız').setStyle(TextInputStyle.Short).setRequired(true)
                ),
                new ActionRowBuilder().addComponents(
                    new TextInputBuilder().setCustomId('oneri_icerik').setLabel('Öneriniz').setStyle(TextInputStyle.Paragraph).setRequired(true)
                )
            );
    ...
    if (interaction.isButton() && interaction.customId === 'cekilis_katil') {
        ...
        if (cekilis.katilimcilar.includes(interaction.user.id)) return interaction.reply({ content: '⚠️ Zaten bu çekilişe katıldınız!', ephemeral: true });
    ...
    if (interaction.isButton() && interaction.customId === 'ticket_olustur') {
        ...
        modal.addComponents(
            new ActionRowBuilder().addComponents(
                new TextInputBuilder().setCustomId('ticket_nick').setLabel('Oyun İçi Kullanıcı Adı').setStyle(TextInputStyle.Short).setRequired(true)
            ),
            new ActionRowBuilder().addComponents(
                new TextInputBuilder().setCustomId('ticket_sorun').setLabel('Sorununuz nedir? (Kısaca anlatın)').setStyle(TextInputStyle.Paragraph).setRequired(true)
            )
        );
    ...
    if (interaction.isButton() && interaction.customId === 'ticket_kapat') {
        ...
        const logMetni = mesajlar.reverse()
    ...
    if (interaction.isModalSubmit() && interaction.customId === 'sikayet_modal') {
        ...
        if (!kanal) return interaction.reply({ content: '❌ Şikayet kanalı bulunamadı.', ephemeral: true });
    ...
    if (interaction.isModalSubmit() && interaction.customId === 'oneri_modal') {
        ...
        if (!kanal) return interaction.reply({ content: '❌ Öneri kanalı bulunamadı.', ephemeral: true });
